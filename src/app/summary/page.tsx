...
function SummaryContent() {
  ...
  useEffect(() => {
    const load = async () => {
      if (archiveId) {
        ...
        // Fall back to live data (time-based crawl over)
        const [event, groups, bars] = await Promise.all([
          getActiveEvent(),
          getGroups(),
          getBars(),
        ]);
  ...
  const groupPhotoCount = data
    ? Object.fromEntries(
        data.groups.map((g) => [
          g.id,
          data.submissions.filter((s) => s.groupId === g.id).length,
        ]),
      )
    : {};
  const groupApprovedCount = data
    ? Object.fromEntries(
        data.groups.map((g) => [
          g.id,
          data.submissions.filter((s) => s.groupId === g.id && s.status === 'approved').length,
        ]),
      )
    : {};
  const barSubmissionCount = data
    ? Object.fromEntries(data.bars.map((b) => [b.id, data.submissions.filter((s) => s.barId === b.id).length]))
    : {};

  const mostPhotosGroup = data
    ? [...data.groups].sort((a, b) => (groupPhotoCount[b.id] ?? 0) - (groupPhotoCount[a.id] ?? 0))[0]
    : null;
  const mostChallengesGroup = data
    ? [...data.groups].sort((a, b) => (groupApprovedCount[b.id] ?? 0) - (groupApprovedCount[a.id] ?? 0))[0]
    : null;
  const hottestBar = data
    ? [...data.bars].sort((a, b) => (barSubmissionCount[b.id] ?? 0) - (barSubmissionCount[a.id] ?? 0))[0]
    : null;
  const totalPoints = data ? data.groups.reduce((sum, g) => sum + g.score, 0) : 0;

  // All photos across all groups (with a url)
    ...
    <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-4 px-4 py-6 pb-24">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {hottestBar && (
              <div className="rounded-[1.5rem] border border-white/10 bg-white/8 p-4">
                ...
                <p className="text-xs text-slate-400">{barSubmissionCount[hottestBar.id] ?? 0} submissions</p>
              </div>
            )}
            <div className="rounded-[1.5rem] border border-white/10 bg-white/8 p-4">
              <p className="text-xl mb-1">⚡</p>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-500">
                        {groupPhotoCount[g.id] ?? 0} photos · {groupApprovedCount[g.id] ?? 0} approved
                      </p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className={`text-xl font-bold tabular-nums ${idx === 0 ? 'text-pink-300' : 'text-slate-100'}`}>
                        {g.score}
                      </p>
                ...
                {allPhotos.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="relative overflow-hidden rounded-xl">
                      ...
                      <span className={`absolute top-1.5 right-1.5 rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${
                        s.status === 'approved' ? 'bg-emerald-500/90' : s.status === 'rejected' ? 'bg-rose-500/90' : 'bg-yellow-500/90'
                      }`}>
          ...
          <Link
            href="/"
            className="flex w-full items-center justify-center rounded-full border border-white/10 bg-white/10 px-4 py-3 text-sm font-semibold text-slate-200 transition hover:bg-white/20"
          >
            Back to home
          </Link>
        </>
